...
export const TypeSkill = ({ index, char, setChar }: { index: number; char: Evertale.Character.State; setChar: React.Dispatch<SetStateAction<Evertale.Character.State>> }) => {
  ...
  const types: string[] = (char.charPassiveSkill[index] as Evertale.Character.PassiveSkill).typeSkill;

  async function keyDownHandler(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Enter") {
      const isThere = passiveSkillTypes.find((skill) => skill === type);
      if (isThere) {
        ...
        setChar({
          ...char,
          charPassiveSkill: char.charPassiveSkill.map((skill, i) => {
            if (i === index) {
              return {
                ...skill,
  ...
  function deleteHandler(e: React.MouseEvent<HTMLSpanElement>) {
    ...
    setChar({
      ...char,
      charPassiveSkill: char.charPassiveSkill.map((skill: Evertale.Character.PassiveSkill, i: number) => {
        if (i === index) {
          return {
            ...skill,
            typeSkill: filtered,
  ...
  return (
    <div id={`Type-Skill-${index}`}>
      ...
      <Input forId="passive-skill-type-input" label="Skill Type Setting" list="option-skill-type" value={type} onChange={(e) => setType(e.target.value)} onKeyDown={(e) => keyDownHandler(e)} />
