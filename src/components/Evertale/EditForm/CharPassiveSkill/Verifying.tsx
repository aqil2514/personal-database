...
export default function Verifying({ char }: { char: Evertale.Character.State }) {
  ...
  const oldCS = init.charPassiveSkill as Evertale.Character.PassiveSkill[];
  const newCS = char.charPassiveSkill as Evertale.Character.PassiveSkill[];
  ...
  const elements: NodeListOf<HTMLParagraphElement> = document.querySelectorAll("#char-passive-skills-result p");
  ...
  return (
    <div className="bg-slate-100 my-4 rounded-xl">
      ...
      <details id="char-passive-skills-result">
          ...
          <div>
            ...
            {oldCS.map((old, i) => (
              <div key={`${old.skillName}`}>
                <>
                  <ComparingData<Evertale.Character.PassiveSkill> subfield="skillName" oldCS={old} newCS={newCS[i]} dataFor="old" title="Skill Name" />
                  <ComparingData<Evertale.Character.PassiveSkill> subfield="typeSkill" oldCS={old} newCS={newCS[i]} dataFor="old" title="Skill Type" />
                  <ComparingData<Evertale.Character.PassiveSkill> subfield="skillDescEn" oldCS={old} newCS={newCS[i]} dataFor="old" title="Skill Description" />
                  <ComparingData<Evertale.Character.PassiveSkill> subfield="skillDescId" oldCS={old} newCS={newCS[i]} dataFor="old" title="Skill Deskripsi" />
                </>
              </div>
            ))}
          </div>
          <div>
            ...
            {newCS.map((newMap, i) => (
              <div key={`${newMap.skillName}`}>
                <ComparingData<Evertale.Character.PassiveSkill> subfield="skillName" oldCS={oldCS[i]} newCS={newMap} dataFor="new" title="Skill Name" />
                <ComparingData<Evertale.Character.PassiveSkill> subfield="typeSkill" oldCS={oldCS[i]} newCS={newMap} dataFor="new" title="Skill Type" />
                <ComparingData<Evertale.Character.PassiveSkill> subfield="skillDescEn" oldCS={oldCS[i]} newCS={newMap} dataFor="new" title="Skill Description" />
                <ComparingData<Evertale.Character.PassiveSkill> subfield="skillDescId" oldCS={oldCS[i]} newCS={newMap} dataFor="new" title="Skill Deskripsi" />
              </div>
            ))}
          </div>
        </div>
